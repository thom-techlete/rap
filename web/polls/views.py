...
def poll_list(request: HttpRequest):
    ...
    closed_polls = Poll.objects.filter(is_active=False).order_by(
...
def poll_detail(request: HttpRequest, pk: int):
    ...
    if request.user.is_authenticated:
        user_votes = Vote.objects.filter(
            poll_option__poll=poll, user=request.user
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                for option_form in formset:
                    if option_form.cleaned_data and not option_form.cleaned_data.get(
                        "DELETE", False
                    ):
                ...
                messages.success(
                    request, f'Poll "{poll.title}" is succesvol aangemaakt!'
                )
...
def vote(request: HttpRequest, pk: int):
    ...
    if not is_active_player(request.user):
        return JsonResponse(
            {"success": False, "error": "Alleen actieve spelers kunnen stemmen."},
            status=403,
        )

    if not poll.is_open:
        return JsonResponse(
            {"success": False, "error": "Deze poll is gesloten voor stemmen."},
            status=400,
        )
    ...
    try:
        ...
        if not option_ids:
            return JsonResponse(
                {"success": False, "error": "Geen opties geselecteerd."}, status=400
            )
        ...
        if set(option_ids) != valid_option_ids:
            return JsonResponse(
                {"success": False, "error": "Ongeldige opties geselecteerd."},
                status=400,
            )
        ...
        if len(option_ids) > 1 and not poll.allow_multiple_choices:
            return JsonResponse(
                {"success": False, "error": "Deze poll staat slechts één keuze toe."},
                status=400,
            )

        with transaction.atomic():
            # Remove existing votes for this poll
            Vote.objects.filter(poll_option__poll=poll, user=request.user).delete()
        ...
        return JsonResponse(
            {"success": False, "error": "Ongeldige data ontvangen."}, status=400
        )
    except Exception:
        return JsonResponse(
            {
