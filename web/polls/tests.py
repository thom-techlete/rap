...
class PollModelTestCase(TestCase):
    ...
    def test_poll_with_end_date(self):
        ...
        future_poll = Poll.objects.create(
            title="Future Poll", created_by=self.staff_user, end_date=future_date
        )
        self.assertTrue(future_poll.is_open)

        # Past end date - should be closed
        past_poll = Poll.objects.create(
            title="Past Poll", created_by=self.staff_user, end_date=past_date
        )
        self.assertFalse(past_poll.is_open)

    def test_poll_options(self):
        """Test poll options creation and ordering"""
        poll = Poll.objects.create(title="Options Test", created_by=self.staff_user)

        option1 = PollOption.objects.create(poll=poll, text="Option 1", order=1)
        option2 = PollOption.objects.create(poll=poll, text="Option 2", order=0)
    ...
    def test_poll_closing(self):
        ...
        poll = Poll.objects.create(title="Closable Poll", created_by=self.staff_user)
...
class PollViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.option1 = PollOption.objects.create(
            poll=self.poll, text="Option 1", order=0
        )
        self.option2 = PollOption.objects.create(
            poll=self.poll, text="Option 2", order=1
        )
...
class PollPermissionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.poll = Poll.objects.create(
            title="Permission Test Poll", created_by=self.staff_user
        )
        self.option = PollOption.objects.create(
            poll=self.poll, text="Test Option", order=0
        )
