...
class ScheduleConfigurationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basis informatie", {"fields": ("name", "task", "reminder_type", "enabled")}),
        (
            "Schema configuratie",
            {
                "fields": (
    ...
    def save_model(self, request, obj, form, change):
        ...
        try:
            ...
            messages.success(
                request,
                f"Configuratie '{obj.name}' opgeslagen en gesynchroniseerd met Celery beat.",
            )
        except Exception as e:
            messages.warning(
                request,
                f"Configuratie opgeslagen, maar synchronisatie met Celery beat is mislukt: {e}",
            )
...
class EventScheduleOverrideAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basis informatie", {"fields": ("event", "configuration", "enabled")}),
        (
            "Schema overrides",
            {
                "fields": (
    ...
    def save_model(self, request, obj, form, change):
        ...
        try:
            ...
            messages.success(
                request,
                f"Override voor '{obj.event.name}' opgeslagen en gesynchroniseerd met Celery beat.",
            )
        except Exception as e:
            messages.warning(
                request,
                f"Override opgeslagen, maar synchronisatie met Celery beat is mislukt: {e}",
            )
...
class PushSubscriptionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Gebruiker informatie", {"fields": ("user", "is_active")}),
        (
