...
def schedule_admin_dashboard(request):
    ...
    overrides = EventScheduleOverride.objects.select_related(
        "event", "configuration"
...
def sync_tasks_manual(request):
    ...
    try:
        sync_periodic_tasks()
        messages.success(
            request, "Periodic tasks successfully synchronized with Celery beat."
        )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not subscription_data:
                return JsonResponse(
                    {"error": "No subscription data provided"}, status=400
                )
            ...
            logger.info(
                f"Push subscription {action} for user {request.user.get_full_name()}"
            )

            return JsonResponse(
                {"success": True, "action": action, "subscription_id": subscription.id}
            )
        ...
        try:
            ...
            if not endpoint:
                ...
                deleted_count = PushSubscription.objects.filter(
                    user=request.user, endpoint=endpoint, is_active=True
                ).update(is_active=False)

            return JsonResponse({"success": True, "unsubscribed_count": deleted_count})
...
def send_test_notification(request):
    ...
    try:
        ...
        if not subscriptions.exists():
            return JsonResponse(
                {"error": "No active push subscriptions found"}, status=400
            )
