...
from .forms import AdminBugReportForm, BugReportForm
...
def help_index(request):
    ...
    context = {"page_title": "Hulp & Ondersteuning"}
...
def bug_report_create(request):
    ...
    context = {"form": form, "page_title": "Bug Rapporteren"}
...
def bug_detail(request, pk):
    ...
    context = {"bug_report": bug_report, "page_title": f"Bug Rapport #{bug_report.id}"}
...
def my_bug_reports(request):
    ...
    context = {"page_obj": page_obj, "page_title": "Mijn Bug Rapporten"}
...
def admin_bug_list(request):
    ...
    bug_reports = BugReport.objects.select_related(
...
def admin_bug_mark_resolved(request, pk):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Bug rapport #{bug_report.id} is gemarkeerd als opgelost."
        )
